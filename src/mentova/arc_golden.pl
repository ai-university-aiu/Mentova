...
agp_load_golden(Game, Count) :-
    ...
    agp_workspace(Game, Workspace),
    ( agp_jspace_ready
    ->  catch(jspace:js_open(Workspace), _, true),
        % A header concept naming the reference, then each line as a numbered step.
        catch(jspace:js_hold(Workspace, golden_reference(Game), 1.0, arc_golden), _, true),
...
agp_hold_line(Workspace, Game, Line, N0, N1) :-
    catch(jspace:js_hold(Workspace, golden_step(Game, N0, Line), 1.0, arc_golden), _, true),
    N1 is N0 + 1.

% agp_jspace_ready: the J-Space workspace predicates are available.
agp_jspace_ready :-
    current_predicate(jspace:js_open/1),
    current_predicate(jspace:js_hold/4).
