    ...
    ->  aggregate_all(count,
            ( a3_rel(Game, Cause, Effect),
              a3_cite(Game, Cite),
                    ...
                    _, fail) ),
            Count)
    % No causal_core: the relations stay queryable via a3_rel/3.
    ;   Count = 0
    ).
    ...
    ->  aggregate_all(count,
            ( a3_hazard(Game, Hazard),
              a3_cite(Game, Cite),
                    ...
                    _, fail) ),
            Count)
    % No causal_core: hazards stay queryable via a3_hazard/2.
    ;   Count = 0
    ).
...
a3_hold_jspace(Count) :-
    ...
    (   a3_defined(jacobian_space:jacobian_space_open(_)), a3_defined(jacobian_space:jacobian_space_hold(_, _, _, _))
    % Open the workspace, guarded.
    ->  catch(jacobian_space:jacobian_space_open(arc3_mind), _, true),
        % Hold each game's identity and each of its relations.
        aggregate_all(count,
            ( a3_jspace_item(Concept, Strength),
              catch(jacobian_space:jacobian_space_hold(arc3_mind, Concept, Strength, arc3_guide), _, fail) ),
            Count)
