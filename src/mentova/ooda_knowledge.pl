...
ooda_bootstrap_ :-
    (   ooda_defined(jacobian_space:jacobian_space_open(_)), ooda_defined(jacobian_space:jacobian_space_hold(_, _, _, _))
    ->  % Open (or reset) the workspace.
        catch(jacobian_space:jacobian_space_open(ooda_mind), _, true),
        % Hold each phase, ingredient, principle, and mapping as a concept.
        forall(ooda_phase(P, _),
            catch(jacobian_space:jacobian_space_hold(ooda_mind, phase(P), 1.0, ooda_boyd), _, true)),
        forall(ooda_orientation_ingredient(I, _),
            catch(jacobian_space:jacobian_space_hold(ooda_mind, orientation(I), 0.9, ooda_boyd), _, true)),
        forall(ooda_principle(Pr, _),
            catch(jacobian_space:jacobian_space_hold(ooda_mind, principle(Pr), 0.95, ooda_boyd), _, true)),
        forall(ooda_hplan_map(O, H),
            catch(jacobian_space:jacobian_space_hold(ooda_mind, maps_to(O, H), 0.85, ooda_boyd), _, true))
...
ooda_stats(stats(Phases, Ingredients, Principles, Held)) :-
    ...
    ( ooda_defined(jacobian_space:jacobian_space_reading(_, _)),
      catch(jacobian_space:jacobian_space_reading(ooda_mind, Reading), _, fail),
      is_list(Reading)
