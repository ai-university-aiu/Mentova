...
cog_pillar(verify_before_act,     co_verify,    'predict a move fatal before spending a real action on it').
cog_pillar(goal_inference,        co_goalinfer, 'hypothesise the unstated win condition from winning deltas').
cog_pillar(efficiency,            co_effic,      'the action-budget governor and RHAE-style scoring').
