...
├── knowledge/      Knowledge bases: commonsense, Gene Ontology, Disease Ontology, school curriculum (ages 0–Grade 12)
...
| `curriculum/curriculum_path_registry.pl` | The **path registry** for the light-pass curriculum import: maps each Reference-Library `SourceId` to the absolute path of its file on disk (16 word walls, 13 standards texts, the curriculum index, and searchable manifests of the 21,128-document bulk EngageNY/Eureka Math corpus). The raw corpus stays on disk; only this compact registry enters git. |
...
**School curriculum (light pass).** The full **age 0 through Grade 12** span is imported youngest-first under the two-tier Knowledge Storage Policy: the clean word walls and standards become understood facts in the head (the lattice, cited — **3,658 facts + 5 CROs across 33 sources**), while the bulk lesson corpus stays on the shelf (the Reference Library — **21,128 documents, ~14 GB**, streamed and searchable on demand, never asserted unread). Bands land one module at a time (elementary ages 0–Grade 5; Middle School / Junior High Grades 6–8; High School Grades 9–12). Full-pass extraction of the bulk corpus is documented with time and disk estimates in `docs/Elementary_School_Full_Pass_Instructions.txt`; testing instructions are in `docs/Chat_on_Elementary_School_topics.txt`; the import and data-structure report (with the appended Middle School and High School work reports) is `docs/Curriculum_Import_Elementary_School.txt`.
