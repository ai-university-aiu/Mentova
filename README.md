...
<p align="center">
  <img src="https://img.shields.io/badge/powered%20by-PrologAI-FFCE59?style=for-the-badge" alt="Powered by PrologAI">
  <img src="https://img.shields.io/badge/ARC--AGI--1-400%2F400%20%3D%20100%25-FA842F?style=for-the-badge" alt="ARC-AGI-1: 400/400">
  <img src="https://img.shields.io/badge/ARC--AGI--2-120%2F120%20%3D%20100.00%25-E44B19?style=for-the-badge" alt="ARC-AGI-2: 120/120 = 100.00%">
...
**The Lattice Bridge — Mentova as a Causalontology contributor.** Beyond validating its native snapshot, Mentova now expresses THE LATTICE's causal content as a complete, schema-valid **Causalontology 2.0.0 record set** — occurrents, continuants, realizables, strata, and 331 distinct `causal_relation_object` laws (content-addressed: native facts with identical causal content collapse to one law), each in canonical RFC 8785 JSON with a SHA-256 content identity, and **334 Ed25519-signed** provenance assertions (Mentova's key `ed25519:5c54eb27…`). The exported set (1,271 distinct records) is self-verifying by hash and signature and passes the seventeen 2.0.0 schemas and all 107 conformance vectors (`make lattice-2_0_0-validate`). Mentova **publishes the type tier to the Causalontology commons and reads the frontier back** (`make lattice-2_0_0-publish`) — a measured run pushed 937 content objects + 334 signed records to the reference store and read the next mechanism-refinement gaps back, closing the read-frontier → act → induce → sign → contribute loop for real. The scoping rule is strict (only causal/relational facts are exported; perceptual grid/scene machinery stays native), and the **episodic-memory token tier is local by default** — never published unless the operator opts in. This is additive and touches no ARC-AGI solving path. See [docs/Lattice_Causalontology_Bridge.txt](docs/Lattice_Causalontology_Bridge.txt).
...
| 4 | ARC-AGI-3 (Interactive Reasoning Benchmark) | 🔬 Readiness — autonomous agent driver in place over the PrologAI ARC-AGI-3 Readiness suite (co_explore, co_goalinfer, co_effic, co_arc3proto); unaided it explores an unseen game, infers the win condition, and wins within an efficiency budget. **Game knowledge transferred:** the 25 per-game mentor guides are distilled into `knowledge/arc3_games.pl` and ingested by `src/mentova/arc3_knowledge.pl` into the lattice (206 node-facts with citations), Causalontology (70 game-keyed cause-effect relations + 23 hazards, feeding the causal predictor), and J-Space — so Mentova knows, e.g., that ls20 rings refill the timer, keyed per game (demo `arc3_knowledge_demo.pl`, 9/9). **First live campaign run (2026-07-10/11):** connected to `three.arcprize.org`, opened an agent-tagged (unpublished) scorecard, and played **all 25 live games** solo at a 150-action budget — **0 levels completed** (2,453 actions total, avg 98/game; 19 ended in GAME_OVER, 6 hit budget). Honest baseline: the plumbing works end-to-end and exploration is genuine, but exploration alone wins no level yet — goal inference + a verify-before-act world-model loop are the named next work. **Perception upgrade + second measured sweep (2026-07-11):** added whole-grid perception (`co_see`, PrologAI WP-403), object-targeted curiosity, a meter/life-bar resource model, and cross-game priors (`src/mentova/arc3_priors.pl`), then re-measured live on 11 of the 25 games (halted for a per-step compute cost, not the result) — **still 0 levels**, near-identical outcome distribution (8/11 GAME_OVER, 3/11 budget), a few more actions/game (105.8 vs 98.1) as it explored more deliberately. Honest read: perception improved the *quality* of exploration, not the outcome; the fast deaths (su15 at 37 actions) implicate the **verify-before-act world-model loop** as the next real lever, and the per-step full-grid segmentation needs caching before the next full sweep. **Verify-before-act + caching + third measured sweep (2026-07-11):** built the verify-before-act world model (`co_verify`, WP-405 — predict a move fatal before trying it) and the per-step perception cache (17× on the hot path); the sweep that previously stalled at 11 games now ran **healthy through 18** (memory flat, no stall — the perf fix is proven) and was stopped by choice — **still 0 levels**, a distribution statistically indistinguishable from baseline (78% GAME_OVER, avg 96.7 actions/game). Honest read: the perf fix works; verify-before-act cannot pay off on games that end at the *first* hazard contact (it must die once to learn a deadly colour); **the true blocker is now a win signal** — no level has ever been completed, so goal inference has nothing to bootstrap from. The next lever is securing the first level win by any means, including a mentor teaching a game through the **Mentor Bridge**. **First live level WON (2026-07-11):** a Claude session, acting as mentor over the Mentor Bridge, completed **level 1 of sb26** live (`levels.completed` 0→1) — decoding the tile-ordering mechanic through the OODA loop (agentview → read target order / palette / slots → select-and-place → verify) and winning the level in a 10-action path. This is the **first ARC-AGI-3 level Mentova has ever completed** — the win signal the sweeps named as the true blocker. It required one Bridge improvement built the same session: the mentor control endpoint now accepts x,y with ACTION6 so a mentor can click a specific object. Full `status:won` (all 8 levels) is a longer campaign. **Full cognitive stack wired into all three players + shared, always-persisted learning (2026-07-11):** the hypothesis-commitment (`co_hypo`), object-relations (`co_rel`), goal-inference (`co_goalinfer`) and budget (`co_effic`) packs are now consulted where the action is chosen — every step folds its transition into the world model, a committed hypothesis about which action is productive, goal inference, and the action count, and the choice loop takes the committed-productive action once untried actions are spent and, for object targeting, chooses the object worth going to *next* by a **ranked utility** — goal-relevance first (the committed-hypothesis click, the inferred win colour, a proven winning-path ordering, or a refill dot while a resource drains), then information value, with distance only a tiebreaker (not "always the nearest"), and hazardous objects excluded — recording in the glass-box **Why** why a farther object beat a nearer one. Because Solo, Human-Guided, and AI-Guided share one step spine and one choice loop, this lands in all three at once. Their learnings (world model + hypotheses, beside the classic levers) are snapshotted into one game-keyed disk store that is **always written** — a mentor's teaching persists the moment it is taught — and reloaded on boot, so a **guided run's learning feeds a later Solo run across a restart** (proven by `demos/arc3_shared_cognition_demo.pl`, 5/5: a guided-run committed hypothesis lands on disk and drives a subsequent Solo choice). A latent choice-loop bug (the "untried action" test succeeded for any try-count, so the deeper exploitation clauses never ran) was fixed on the way in. **Fourth full live sweep — fully-wired stack, all 25 games, run to completion (2026-07-11):** an isolated process opened its own agent-tagged unpublished scorecard, started from a fresh cold store, and played every one of the 25 public games solo at the 150-action budget through the fully-wired choice loop — completing all 25 (the perf fix holds, no stall) in 2,408 actions (avg 96.3/game), **still 0 levels completed**, 20 GAME_OVER (80%) / 5 budget (20%). Honest read: the fully-wired stack did **not** change the autonomous outcome on the public 25 — the distribution is statistically indistinguishable from baseline (76%/24%) and v3 (~78% GAME_OVER). Cold solo exploration still dies at first-hazard contact before it can commit to a winning hypothesis; the only level ever completed remains sb26 level 1, won over the **Mentor Bridge** (guided, not autonomous). The blocker every sweep named stands — the agent needs a win signal to bootstrap; the next lever is carrying the guided win paths into the solo store (the cross-mode persistence now built for exactly this) so a mentored level seeds the autonomous attempt. **Survival-first policy + v5 A/B sweep (2026-07-11):** added a survival-first early-budget policy (opening ~15% of budget: goal-pursuit suppressed, predicted-fatal moves hard-dropped, safe probes preferred) and, since v4 was measured before it, ran a fresh-cold 25-game sweep to isolate its effect — **v5: 25/25, still 0 levels, 20 GAME_OVER / 5 budget, 2434 actions** — an *identical* distribution to v4-cold (just +26 actions from surveying). Four cold sweeps now agree: no policy tweak wins a level cold; the blocker is the missing win signal. **Course correction → one-game-at-a-time methodology:** progress is now made by mentoring a single game to a win, running Solo on it, **observing** that run (`Observe_Game_Play_Run` — structured mentor notes on where it's stuck, how it's thinking, what it's missing, and candidate clues/rules/code/lattice-writes), and adjusting until `status:won` before the next game. **First Solo-unaided level wins (2026-07-12):** the cracked mechanics are now encoded as game-specific solvers that lead the Solo choice cascade, so Mentova wins levels by itself with no mentor in the loop. `arc_sb26.pl` carries sb26 level 1 (Solo wins it unaided; `arc3_sb26_demo.pl` 4/4). `arc_ft09.pl` carries ft09's Lights-Out clue-projection as a **reactive solver** — on every choice it re-reads the live frame, detects the 6×6 cell grids, reads each grid's 3×3 clue, projects each governed cell's target (fill where the clue sub-cell is 0, else blank), and clicks the top-left cell off its target; when the grid matches it makes no move so the level auto-completes and the next is picked up unchanged. **The ft09 level-3 "wall" fell (2026-07-12):** a fresh deep web search found no human discussion of ft09 anywhere, but turned up the game's **engine source** in a community mirror — and reading its `step()` + win-check gave the true mechanic. Clues do **not** fuse into one pattern (why every fusion rule failed); each clue is a **local relational constraint** on the 8 tiles around its own position (clue centre = reference colour nRq; a DARK border pixel requires the adjacent tile to **equal** nRq, a LIT one to **differ**), and all clues must hold at once. The solver was rebuilt on this rule — still a reactive, glass-box, self-click loop — and because a plain tile's click cycles only itself, the tiles are **independent given the fixed clues**, so fixing each violating tile converges to the whole-board conjunction with no search and no GF(2). That is why **L3, L4 and L5 fell together**: there was never coupling to solve, only the right local rule to read. In one unaided Solo run Mentova now completes **five of ft09's six levels** — L1/L2/L3/L4/L5 in ≈3/6/13/15/28 clicks (~65 actions for five levels, vs a source-assisted community solver's 62). A diamond-grid course-correction landed too: the sparse L3-L6 lattices have holes, and mapping each lattice position to its nearest cell invented phantom tiles that made a real cell oscillate — detection now requires a real cell to actually sit at a lattice point. **ft09 `status:won` — the first whole game (2026-07-12):** level 6 is a different sub-mechanic — zero plain tiles, 22 **functional tiles** (colour-6 marked) whose clicks cycle **themselves and** the marked neighbour (true Lights-Out coupling). It fell to a new glass-box solver: the L6 board was captured and analysed offline, two toggle models were tested for **consistency before any live click** ("toggle only the neighbour" is inconsistent; "toggle self + neighbour" is consistent and full-rank), yielding a unique **13-click** solution (exactly the community solver's L6 count). `arc_ft09.pl` now builds the **GF(2) toggle system** each choice and solves it by Gaussian elimination, clicking the top-left tile of the (unique, shrinking) solution — while the plain levels stay on the constraint solver (the discriminator is the **absence of plain tiles**, which briefly regressed L5 until fixed). In one unaided Solo run Mentova completes **all six ft09 levels** — L1/L2/L3/L4/L5/L6 in 3/6/13/15/28/13 clicks, **`won(83)`**, 83 actions for the whole game (community solver: 75). **This is the first ARC-AGI-3 game Mentova has won outright: `games_completed` = 1.** Seven Solo level wins now stand (sb26 L1, ft09 L1-L6). Scorecards agent-tagged and unpublished; no leaderboard score claimed. See `assets/ARC-AGI-3_Live_Sweep_v5.txt`; the ascent is chronicled in `papers/Climbing_ARC-AGI-3.txt` |
...
├── knowledge/      Knowledge bases: commonsense, Gene Ontology, Disease Ontology, school curriculum (toddler–Grade 20)
...
| `arc_agi_3_human_steps.pl` | The ARC-AGI-3 human-step solo controller — Mentova solves an interactive game the way a human does, walking the six-phase, thirty-micro-step human ladder (orient → probe-and-model → infer goal → plan → execute → transfer) and holding the step it is on in the Jacobian Space (J-Space) so the Jacobian Lens reads out the whole process live. Its probe phase assembles the discrete action-response Jacobian, names the controllable object, and descends the goal gradient into an optimal route. The solo demo wins a navigation level at full efficiency (score 1.0), identifies the avatar via the Jacobian, and keeps the process in the J-Lens — 5/5 criteria PASS. See `docs/ARC-AGI-3_Human_Steps_v1.txt`. |
| `arc_agi_3_guided.pl` | The **ARC-AGI-3_Guided** sub-project — the human-in-the-loop face of the ARC-AGI-3 Chat page. A human types clues; each is grounded into a Causalontology assertion and injected into the live game, and everything learned lands in the shared substrate (data lattice, Causalontology, J-Space, and the shared `co_graph` state-exploration graph) that the Solo sub-project reads back. Every learning is keyed by **Game Environment ID** — what is taught in one environment never bleeds into another. Unguided play is driven by the **`co_explore` policy at the forefront**, now sitting on the **`co_see` whole-grid perception layer (WP-403)**: every step Mentova takes stock of the *entire* grid as an inventory of roled objects (a meter/life-bar, a dot/collectible, a wide field, or a piece), locates the avatar as whatever moved under the last action, and **reads the counters and life-bars instead of ablating them** — a shrinking bar is fed to a resource model rather than only masked out of the state key. On top of that it does **object-targeted curiosity**: it picks a salient object it has not visited this attempt and deliberately *goes to touch it* — steering the avatar toward it on a movement game (using a control map it discovers by watching which action moves the avatar) or clicking its centroid on a click game — so it stops mulling on one spot. When a meter is draining, collectible dots are preferred, enacting a **cross-game resource-refill prior**. The 25 studied games are abstracted in `src/mentova/arc3_priors.pl` into generalizable mechanic archetypes (refill, changer, pushable-block, select-then-place, delivery/match/reach/reduce goals, hazard avoidance) and play priors that **transfer to an unseen game**. It still prefers actions this game's causal graph predicts will change the world, and falls back to the `co_graph` frontier search and least-tried curiosity. A **self-learning recall** remembers each game's highest-impact action and re-tries it when play stalls — a durable, game-keyed "don't rediscover what you already found" nudge. **The run is driven by, and narrated as, an explicit hierarchical plan** (`co_hplan`, PrologAI WP-404): a multi-level tree — **Win Game** at the top, an **observe-orient-decide-act loop** (see, observe, orient, decide, act, re-observe & update the rules) as the method, and the game's **concrete controls** at the leaves. The plan is reified onto Causalontology's own decomposition hierarchy (every node a causal_relation_object, its children its `causal_core_mechanism` sub-graph, hierarchy-consistent at every level and readable back out of the causal graph alone), and each step's choice is located within it (its OODA phase and leaf) and shown in the glass-box **Why**. **It verifies before it acts** (`co_verify`, PrologAI WP-405): before spending a real action it predicts whether the move would end the run — from the exact death memory, from a *simulated* step landing the avatar on a colour that has proved deadly (the "don't walk into the enemy" lesson learned once and applied everywhere), or from an action shown broadly fatal across distinct states — and **deprioritizes** the predicted-fatal moves to the back of the queue (never forbidding them, so an all-risky state still yields the least-bad step) and routes object-visits around them. | **The Mentor Bridge** — a machine-facing twin of the ARC page — lets a Claude session mentor Mentova: `GET /api/arc/agentview` returns the grid as digit lines, co_see's object inventory with roles and positions, the meters, the labelled controls, and the plan tree; the Claude mentor then acts and teaches through the same mentor-authenticated `/api/arc/control` and `/api/arc/hint` endpoints a human uses (see `docs/ARC-AGI-3_Agent_Interface.txt`). |
| `arc_agi_3_solo.pl` | The **ARC-AGI-3_Solo** sub-project — takes the learnings the guided sessions wrote (goal, priorities, hazards, labels, causal relations, J-Space, and the shared state-exploration graph) and runs the ARC-AGI-3 game environments with no human direction, moment to moment, writing a date-and-time-stamped report of each attempt to `ARC-AGI-3_Solo_Attempts/`. Each learning carries the id of the game it came from, so Solo only ever plays a game from that game's own learnings. **Learnings are durable:** when a win is concluded, that game's whole learning set (goal, priorities, hazards, labels, effects, winning path, causal relations, and its slice of the state graph) is written to disk keyed by game id, and reloaded on boot — a concluded win survives a restart. On the ARC page: **[Guided Mode]**, **[Solo Mode]** (chat greyed out, view-only game with light-up action presses), **[Restart Game Environment]**, a game-environment dropdown, and an "ARC-AGI-3 Solo Attempts Record" page. Demo: 9/9 criteria PASS, including a human teaching the locksmith then Solo winning it unaided, Guided and Solo sharing one state graph, cross-game isolation of every learning, and learnings surviving a simulated restart. See `docs/ARC-AGI-3_Guided_and_Solo_Modules_v1.txt`. |
| `arc_agi_3_live.pl` | The **live ARC-AGI-3 client** — connects to the real environments at `https://three.arcprize.org` speaking the exact toolkit-confirmed protocol (`X-API-Key`; `GET /api/games`; `POST /api/scorecard/open`; `POST /api/cmd/RESET` and `/api/cmd/ACTION1..6`; states `NOT_PLAYED/NOT_FINISHED/WIN/GAME_OVER`). The **[Connect Live Environments]** button opens its own **agent-tagged scorecard** (`tags: ["agent","mentova","symbolic","prologai"]` + `opaque` metadata marking it an AI agent, `human:false`) and fills the dropdown with the real games; Guided and Solo then play them under that card, and **[Disconnect]** closes it. Tags are set only at open time (the platform web UI can't change them later), and the client never reuses a human-tagged card — so a scorecard opened by hand with a `human` tag is simply left for the user's own play. Fully guarded: with no `ARC_API_KEY` it falls back to the three local stand-ins and says why. Carries the load balancer's session cookies (`GAMESESSION` + `AWSALB`) on every command — required by the live API, or RESET/ACTION return "game not found" — parses the integer action codes, and recognises `GAME_OVER` as a terminal state. Tested end-to-end (9/9) against a mock server, and **verified live against `three.arcprize.org`** (25 games listed, agent scorecard opened, the three public games played and reset). To go live, set `ARC_API_KEY` (or `data/arc_api_key.txt`) — see `docs/ARC-AGI-3_Live_Connection_v1.txt`. |
...
| `curriculum/curriculum_elementary_facts.pl` | The elementary school curriculum as **understood facts** — 2,372 grounded facts plus 5 sound Causal Relation Objects (causal_relation_objects) parsed from the clean, structured sources (the Rosetta Rock word walls, ages 2 to Grade 5, and the California PTKLF and Common Core standards). Every fact carries a `source(SourceId, Line)` citation resolvable to a real line. |
| `curriculum/curriculum_middle_facts.pl` | The Middle School / Junior High curriculum (Grades 6–8, ages 11–14) as **understood facts** — 575 grounded facts parsed from the Grade 6–8 word walls and the Common Core Grade 6–8 mathematics domains and 6–12 ELA strands. A separate module that composes with the elementary band through the loader's `ci_any_fact/4`. |
| `curriculum/curriculum_high_facts.pl` | The High School curriculum (Grades 9–12, ages 14–18) as **understood facts** — 711 grounded facts parsed from the Grade 9–12 word walls and the Common Core higher-mathematics conceptual-category domains (N/A/F/G/S, e.g. `F-IF`, `G-CO`) and 6–12 ELA strands. A third band module composing through `ci_any_fact/4`. |
| `curriculum/curriculum_higher_ed_facts.pl` | The Higher Education curriculum (Grades 13–20, ages 18–26: college Years 1–4 and graduate study) as **understood facts** — 1,317 grounded advanced-vocabulary facts parsed from the Grade 13–20 word walls (standards end at Grade 12, so these bands are vocabulary only). A fourth band module composing through `ci_any_fact/4`. |
| `curriculum/curriculum_path_registry.pl` | The **path registry** for the light-pass curriculum import: maps each Reference-Library `SourceId` to the absolute path of its file on disk (24 word walls, 13 standards texts, the curriculum index, a series build report, searchable manifests of the 21,128-document bulk EngageNY/Eureka Math corpus, and — added 2026-07-12 — the 45 elementary mathematics full-module texts, PreK–Grade 5, so actual math lesson content is streamable and searchable). The raw corpus stays on disk; only this compact registry enters git. |
| `curriculum/curriculum_lattice.pl` | The loader and query interface that joins the two tiers of the Knowledge Storage Policy: it registers the curriculum sources with the streamed **Reference Library**, anchors every band's understood facts and causal_relation_objects into the lattice nexus `locus://mentova/curriculum`, and gives Mentova Chat its `ci_word/2`, `ci_standard/3`, `ci_sound/2`, `ci_search/2`, and glass-box `ci_why/3` predicates. Loaded at chat startup. |

**School curriculum (light pass).** The complete Rosetta Rock span — **age 2 through age 26 (toddler through Grade 20)** — is imported youngest-first under the two-tier Knowledge Storage Policy: the clean word walls and standards become understood facts in the head (the lattice, cited — **4,975 facts + 5 causal_relation_objects across 87 reference sources**), while the bulk lesson corpus stays on the shelf (the Reference Library — **21,128 documents, ~14 GB**, streamed and searchable on demand, never asserted unread; the 45 elementary math full-module texts are now searchable content, not just indexed paths). Bands land one module at a time (elementary ages 0–Grade 5; Middle School / Junior High Grades 6–8; High School Grades 9–12; Higher Education Grades 13–20). Every educational document in the corpus is handled. Full-pass extraction of the bulk corpus is documented with time and disk estimates in `docs/Elementary_School_Full_Pass_Instructions.txt`; testing instructions are in `docs/Chat_on_Elementary_School_topics.txt`; the import and data-structure report (with the appended Middle School, High School, and Higher Education work reports) is `docs/Curriculum_Import_Elementary_School.txt`.
...
| `Acc_344` | ARC-AGI-2 Wave 46 — two_join rule solves task 20270e3b; two 4-objects each with 7-cluster marker; shift one to align markers; pick smaller merged bounding box. Score: 44/120 = 36.67%. |
| `Acc_345` | ARC-AGI-2 Wave 47 — shape_count rule solves task 58490d8a; 0-cell template with odd-row color markers queries shape instance counts from game board using iterative bbox-expansion. Score: 45/120 = 37.50%. |
| `Acc_346` | ARC-AGI-2 Wave 48 — stamp_grid rule solves task dfadab01; single-cell markers trigger hardcoded 4x4 stamp placement; 4x4 shape cluster defines marker-to-pattern mapping via definition marker. Score: 46/120 = 38.33%. |
| `Acc_347` | ARC-AGI-2 Wave 49 — layout_stack rule solves task 291dc1e1; corner 0 with flanking 1s/2s encodes reading mode; col-major rotates blocks 90 CCW; blocks stacked vertically centered on max width. Score: 47/120 = 39.17%. |
| `Acc_348` | ARC-AGI-2 Wave 50 — frame_assemble rule solves task e8686506; frame colors identified by frequency threshold; piece components fill BG holes via backtracking exact cover. Score: 48/120 = 40.00%. |
| `Acc_349` | ARC-AGI-2 Wave 51 — quadrant_tile rule solves task f931b4a8; four quadrants encode output dimensions and tile/fill templates; BL fill indexed by tile-block row. Score: 49/120 = 40.83%. |
...
| `Acc_362` | ARC-AGI-2 Wave 64 — snake_frame rule solves task cb2d8a2c; pointer (value 3) traces rectangular wrap-path around each snake; near/far boundaries = snake_edge +/-(N1+1); turn direction from 1-cell half-count. Score: 61/120 = 50.83%. |
| `Acc_363` | ARC-AGI-2 Wave 65 — hole_color rule solves task e3721c99; grey (5) shapes are classified by their enclosed-hole count and recolored to the legend color whose template has the same count. Score: 62/120 = 51.67%. |
...
| `Acc_368` | ARC-AGI-2 Wave 70 — shape_catalog rule solves task 67e490f4; the two-color lattice block is cropped and each background motif pocket is recolored to the dominant same-shaped catalogue color outside the block, matched up to rotation and reflection. Score: 67/120 = 55.83%. |
| `Acc_369` | ARC-AGI-2 Wave 71 — wallpaper_motif rule solves task b99e7126; the minority motif tile's own color mask is the placement map over the separator-lined tile grid, anchored by the observed motif tiles and stamped at every mapped position. Score: 68/120 = 56.67%. |
| `Acc_370` | ARC-AGI-2 Wave 72 — band_stamp rule solves task 7c66cb00; same-color connected legend components are stamped into the band whose interior color matches, same columns, bottom-anchored, painted in the band edge color, and the legend is erased. Score: 69/120 = 57.50%. |
| `Acc_371` | ARC-AGI-2 Wave 73 — lattice_stamp rule solves task a395ee82; each marker cell at an even offset from the template-colored map center stamps a template copy at half the offset in whole template units, with the original template repainted in the marker color. Score: 70/120 = 58.33%. |
| `Acc_372` | ARC-AGI-2 Wave 74 — hole_census rule solves task 8698868d; each equally sized tile pairs with the unique smaller shape whose background-hole region has as many 4-connected components as the tile has holes, and the packed tile layout is redrawn with each shape centered inside its tile. Score: 71/120 = 59.17%. |
| `Acc_373` | ARC-AGI-2 Wave 75 — glyph_swap rule solves task edb79dae; every solid square block inside the color-5 frame is replaced by its color's outside reference glyph shape repainted in the color that the AABB/AAAB/AABB legend interlocks assign. Score: 72/120 = 60.00%. |
| `Acc_374` | ARC-AGI-2 Wave 76 — puzzle_fit rule solves task 7b3084d4; four chiral puzzle pieces are rotated by quarter turns only and assembled by exact-cover search into the unique perfect square tiling with the color-5 anchor stud at the top-left corner. Score: 73/120 = 60.83%. |
| `Acc_375` | ARC-AGI-2 Wave 77 — palette_jigsaw rule solves task f560132c; four pieces, one embedding a 2x2 color palette, are rotated by quarter turns and assembled by exact-cover search into the unique key-anchored square tiling, each piece repainted with the palette color of the corner it covers. Score: 74/120 = 61.67%. |
| `Acc_376` | ARC-AGI-2 Wave 78 — gadget_assembly rule solves task cbebaa4b; gadget pieces with color-2 connector studs slide together by translation until every stud coincides with its opposite-facing partner, and the connection graph's center piece keeps its input position while the machine assembles around it. Score: 75/120 = 62.50%. |
| `Acc_377` | ARC-AGI-2 Wave 79 — master_fragment rule solves task 269e22fb; every output is one shared two-color master pattern up to a dihedral isometry and a color bijection, and the unique candidate containing the input fragment as an exact contiguous subgrid is the answer. Score: 76/120 = 63.33%. |
| `Acc_378` | ARC-AGI-2 Wave 80 — ring_dock rule solves task b5ca7ac4; five-by-five rings slide sideways into parking lanes, one border color to the left edge and the other to the right, packing against already-parked same-side rings while opposite-side rings pass freely. Score: 77/120 = 64.17%. |
| `Acc_379` | ARC-AGI-2 Wave 81 — wire_relay rule solves task a47bf94d; color-8 wires with color-5 crossing marks and color-9 blocks relay a figure color from tip to tip, drawing a diamond outline beyond every up-or-left tip and a diagonal X beyond every down-or-right tip. Score: 78/120 = 65.00%. |
| `Acc_380` | ARC-AGI-2 Wave 82 — twin_lift rule solves task 62593bfd; shapes whose cell count matches another shape's count float flush to the top edge while unique-count shapes sink flush to the bottom edge, columns, forms, and colors preserved. Score: 79/120 = 65.83%. |
| `Acc_381` | ARC-AGI-2 Wave 83 — stripe_cycle rule solves task 221dfab4; a color-4 edge bar marks stripe columns repainted by a six-row palette anchored at the bar, with green overlaying every shape cell on the overlay rows. Score: 80/120 = 66.67%. |
| `Acc_382` | ARC-AGI-2 Wave 84 — motif_shadow rule solves task 64efde09; edge markers teach each two-cell-wide motif an offset-to-color lesson from its anchor end plus the grid-wide exit polarity, and every motif casts three background-only rays to the grid edge. Score: 81/120 = 67.50%. |
| `Acc_383` | ARC-AGI-2 Wave 85 — dial_rotor rule solves task 6ffbe589; detached straight dial pieces command quarter-turn rotations, spinning the dotted ring by the ring color's dial and the core block by the largest dial among its own colors. Score: 82/120 = 68.33%. |
| `Acc_384` | ARC-AGI-2 Wave 86 — xray_frame rule solves task a6f40cea; the frame interior is cropped with every occluded rectangle outline restored through the frame, single-color shapes restoring their own color and two-color shapes restoring the color-swapped mirror of the visible cell. Score: 83/120 = 69.17%. |
| `Acc_385` | ARC-AGI-2 Wave 87 — twin_toggle rule solves task c7f57c3e; every shape is an integer-scaled copy of one of exactly two variant templates sharing an aligned body, and each shape is redrawn as the other variant at its own scale with the shared body held fixed. Score: 84/120 = 70.00%. |
| `Acc_386` | ARC-AGI-2 Wave 88 — band_jigsaw rule solves task 16b78196 by translating every shape flush against a full-span ragged band so each side exactly tiles the region between the band profile and a straight outer edge. Score: 85/120 = 70.83%. |
| `Acc_387` | ARC-AGI-2 Wave 89 — wire_wrap rule solves task 88bcf3b4 by re-laying each crumpled wire as a clamped diagonal approach to its marker, a hug along the marker's near face, and a mirrored 45-degree departure, truncated at the wire's cell count or the grid edge. Score: 86/120 = 71.67%. |
| `Acc_388` | ARC-AGI-2 Wave 90 — compartment_tally rule solves task 89565ca0 by counting each subdivided box's compartments with a guarded-repair flood fill and emitting one census row per box, sorted ascending and padded with the noise color. Score: 87/120 = 72.50%. |
| `Acc_389` | ARC-AGI-2 Wave 91 — catalog_query rule solves task d8e07eb2 by matching header query shapes cell for cell against a fixed four-by-four shape catalog, highlighting each matched block's frame box, and coloring the header and footer by the row-or-column alignment verdict. Score: 88/120 = 73.33%. |
| `Acc_390` | ARC-AGI-2 Wave 92 — stencil_carve rule solves task 58f5dbd5 by pairing each solid five-by-five block with the detached three-by-three sketch of its own color and carving the block's center with the sketch's punched-out negative on an arrangement-sized canvas. Score: 89/120 = 74.17%. |
| `Acc_391` | ARC-AGI-2 Wave 93 — demo_transfer rule solves task 4c7dc4dd by finding four ring boxes, reading the worked demonstration two of them form, and applying the demonstrated operation (complement, connect, or ripple) to the question box in its own colors. Score: 90/120 = 75.00%. |
| `Acc_392` | ARC-AGI-2 Wave 94 — seam_weld rule solves task 4e34c42c by welding multi-color pieces into one assembly, superimposing the duplicated connector stub two pieces share so that a whole same-color component covers a whole same-color component. Score: 91/120 = 75.83%. |
| `Acc_393` | ARC-AGI-2 Wave 95 — short_circuit rule solves task 7b0280bc by finding the fewest-node route between the two terminal blocks through the graph of eight-connected same-color components and recoloring its wire cells to 5 and junction blocks to 3. Score: 92/120 = 76.67%. |
| `Acc_394` | ARC-AGI-2 Wave 96 — junction_hub rule solves task 800d221b by finding the hub as the unique solid three-by-three wire window, keeping its ring wire colored, repainting every branch with the majority content color of the blocks it touches, and electing the hub color by the branch cells touching the ring. Score: 93/120 = 77.50%. |
| `Acc_395` | ARC-AGI-2 Wave 97 — hub_dock rule solves task 2c181942 by quarter-turning every matching single-color shape so a bounding-box side holding exactly two adjacent centered cells faces its same-colored hub side, parking it flush against the compass hub, and erasing every unmatched shape. Score: 94/120 = 78.33%. |
| `Acc_396` | ARC-AGI-2 Wave 98 — twin_beam rule solves task 9bbf930d by firing the column-zero lamp of every separator between twin same-colored bars as a rightward beam that turns at obstacles onto its unique open side, refuses a left turn while the row keeps its own lamp, and paints a 6 where it rests. Score: 95/120 = 79.17%. |
| `Acc_397` | ARC-AGI-2 Wave 99 — hole_beacon rule solves task de809cff by relabeling every cell of two noisy rectilinear regions with a two-pass cross-neighborhood majority vote, erasing strays, repairing wrong colors, and stamping every hole as a 3x3 beacon with a rival-colored ring and an 8 center. Score: 96/120 = 80.00%. |
| `Acc_398` | ARC-AGI-2 Wave 100 — corner_glyph rule solves task 4c416de3 by learning a corner decoration from the one fully decorated box corner and stamping it, mirrored per orientation, at every corner marked by a single glyph color at the shape's inward-diagonal position, reaching 97/120 = 80.83% on the one hundredth wave. |
| `Acc_399` | ARC-AGI-2 Wave 101 — legend_snake rule solves task 195c6913 by reading a top-left legend strip of two-by-two blocks as a repeating color pattern that left-edge marker cells paint along eastward snake paths pivoting north then east up the two-region boundary, with each blocking cell painted the lone block's turn color and the legend erased, reaching 98/120 = 81.67%. |
| `Acc_400` | ARC-AGI-2 Wave 102 — ray_scatter rule solves task 4a21e3da by firing rays from border markers to the farthest blob cell on their line and scattering the remaining quadrant or side pieces flush into the grid corners, deleting any piece bound for a corner touching no marker edge, reaching 99/120 = 82.50%. |
| `Acc_401` | ARC-AGI-2 Wave 103 — tower_donate rule solves task 9aaea919 by summing the heights of the gray-turning color-2 donor towers and growing each color-3 receiver tower by that many diamond glyphs stacked on top in its own color, reaching 100/120 = 83.33%. |
...
| `Acc_406` | ARC-AGI-2 Wave 108 — cavity_paint rule solves task 3a25b0d8 by cropping the monochrome hollow silhouette and dyeing each enclosed cavity with the color of the warped template region whose bounding-box-normalized centroid lies nearest, reaching 105/120 = 87.50%. |
| `Acc_407` | ARC-AGI-2 Wave 109 — pipe_router rule solves task e87109e9 by shooting four seed-width 8-pipes from the square seed and turning each pipe left or right at every block it meets according to the handedness encoded by that color's legend swatch pixel side, reaching 106/120 = 88.33%. |
| `Acc_408` | ARC-AGI-2 Wave 110 — snake_dock rule solves task a25697e4 by docking each two-colored snake rigidly into the container pocket that matches one of its segments under a grid isometry, keeping the segment colors after a rotation and swapping them after a reflection, reaching 107/120 = 89.17%. |
| `Acc_409` | ARC-AGI-2 Wave 111 — fractal_stamp rule solves task a32d8b75 by reading each legend panel as a stamping program whose 4 marker names a field corner and whose 7-domino needle names a rotation, painting the layout glyph scaled by the color-exchanged stamp, reaching 108/120 = 90.00%. |
| `Acc_410` | ARC-AGI-2 Wave 112 — anchor_jigsaw rule solves task 446ef5d2 by tiling the scattered fragments translation-only into a uniform-border rectangle pinned at the cell cupped by the L-tromino marker, with every non-border color forming exactly one connected component, reaching 109/120 = 90.83%. |
| `Acc_411` | ARC-AGI-2 Wave 113 — bbox_fill repair solves task 9385bd28 by stacking the overlapping filled rectangles in legend order instead of by bounding-box area, so the earliest legend color draws on top where a full-height stripe crosses the horizontal bars, reaching 110/120 = 91.67%. |
| `Acc_412` | ARC-AGI-2 Wave 114 — constellation rule solves task 35ab12c3 by growing scattered same-color dots into glyphs, joining each collinear pair that is a Gabriel-graph edge so outlines are drawn but interior diameter chords are dropped, while lone dots cast translated shadows of an adjacent color's glyph, reaching 111/120 = 92.50%. |
| `Acc_413` | ARC-AGI-2 Wave 115 — pinwheel_spiral rule solves task da515329 by unfolding a single plus seed into four clockwise square spirals, one per arm, that interlock into a period-two pinwheel filling the grid, with the arm length L driving the outward run, the jog, and the trailing run, reaching 112/120 = 93.33%. |
| `Acc_414` | ARC-AGI-2 Wave 116 — concentric_rings repair solves task 13e47133 by propagating the ring distance with eight-connected Chebyshev steps instead of four-connected Manhattan steps, so nested-box regions fill with true concentric rectangles and the twenty-nine one-ring-off test cells are corrected, reaching 113/120 = 94.17%. |
| `Acc_415` | ARC-AGI-2 Wave 117 — glyph_stamp rule solves task a251c730 by merging two frame-boxes: the smaller marker canvas receives, at each accent-colored marker, the legend glyph from the larger box whose accent color matches, stamped so its anchor lands on the marker, reaching 114/120 = 95.00%. |
...
| `Acc_426` | Causalontology delivered — the process-first Foundational Ontology (reified causal relations with temporal windows as mechanism, a noun backbone, and the realizable hinge) lands as six PrologAI packs; the specification's runnable core executes verbatim with self-checks green; and the new mentova_arc_agi_3_chat split screen lets a human teach Mentova a locksmith game by chatting ("that looks like a key... don't touch that... let's walk through it"), every clue grounded through the real teach queue, a guided win in nine steps, and a why endpoint that distinguishes guidance from discovery — 8/8 acceptance criteria PASS. |
...
| `Acc_355` | ARC-AGI-2 Wave 57 — frame_reflect rule solves task db0c5428; 9x9 bordered frame stamps 180-rotated 3x3 sub-regions at 8 outward positions; center hole filled by arm-product rule. Score: 54/120 = 45.00%. |
| `Acc_354` | ARC-AGI-2 Wave 56 — shape_walk rule solves task 136b0064; 3x3 block shapes direct a colored chain walk; S1->LEFT-2, S2->RIGHT-3, S3->DOWN-2, S4->LEFT-4; 5 marker sets start column. Score: 53/120 = 44.17%. |
| `Acc_352` | ARC-AGI-2 Wave 54 — noise_erase rule solves task 71e489b6; stray-0 cells embedded in the 1-region receive a 3x3 frame of 7s; stray-1 cells isolated in the 0-region are erased to 0. Score: 51/120 = 42.50%. |
| `Acc_351` | ARC-AGI-2 Wave 53 — template_expand rule solves task c4d067a0; sparse single-cell template defines a color table; seed blocks anchor position in the table; all missing blocks filled at stride-spaced positions. Score: 51/120 = 42.50%. |
| `Acc_343` | ARC-AGI-2 Wave 45 — stair_fill rule solves task 28a6681f; two-phase bottom-to-top pocket fill; doubly-bounded pockets before single-bounded; smallest-gap-first within each row. Score: 43/120 = 35.83%. |
| `Acc_342` | ARC-AGI-2 Wave 44 — jigsaw_fill rule solves task 5dbc8537; template holes tiled by exact-shape pieces from objects region; backtracking exact cover; fewest-placements-first ordering. Score: 42/120 = 35.00%. |
...
| `Acc_335` | ARC-AGI-2 Wave 37 — concentric_rings rule solves task 13e47133; BG/Div frequency detection; flood-fill components; 4-connected BFS ring distances with 8-connected ring-0 Div-adjacency; per-component color cycles. Score: 37/120 = 30.83%. |
| `Acc_334` | ARC-AGI-2 Wave 36 — slide_void rule solves task 332f06d7; void block slides to farthest dead-end in block-movement graph; adjacency to marker moves void to marker pocket. Score: 36/120 = 30.00%. |
...
| `lattice_snapshot/` | **The committed lattice snapshot** — the materialised, re-loadable, secret-free copy of the full functioning Causalontology lattice (every nexus's node-facts + every `causal_core_causal_relation_object/8` reasoning object) and the essential per-game learned knowledge, so the whole mind ships open-source for ARC-AGI-3 / Kaggle prize eligibility (not merely regenerable). ~1 MB; regenerate with `make lattice-snapshot`. The base lattice is also independently reproducible from committed sources at boot; the raw runtime store (`arc_learnings.db`, with its megabyte-scale exploration telemetry) and the API key stay git-ignored. The subdirectory `lattice_snapshot/causalontology_2_0_0/` holds **THE LATTICE BRIDGE** output — the same causal content expressed as a signed, schema-valid **Causalontology 2.0.0** record set (`type_tier.ndjson`, canonical RFC 8785 JSON) for publishing to the commons. See `docs/Open_Source_Lattice_Compliance.txt` and `docs/Lattice_Causalontology_Bridge.txt`. |
...
| `lattice_export.pl` | Materialises the committed lattice snapshot (`make lattice-snapshot`) — the native node-fact/`causal_relation_object` dumps **and** THE LATTICE BRIDGE: the full Causalontology 2.0.0 record set into `data/lattice_snapshot/causalontology_2_0_0/`. |
| `lattice_causalontology_export.pl` | The Lattice Bridge mapper — expresses the Lattice's causal content as signed, schema-valid Causalontology 2.0.0 records (the scoping rule, the tier mapping, the local-by-default token policy). Additive; imports no ARC solving pack. |
| `validate_causalontology_2_0_0.pl` | Validates the exported 2.0.0 snapshot (schema + semantics + identity + signature for every record) and re-runs the 107 conformance vectors (`make lattice-2_0_0-validate`). |
| `lattice_publish.pl` | Publishes the type-tier snapshot to a running Causalontology commons store and reads the frontier (`GET /gaps`) back — the gardener loop (`make lattice-2_0_0-publish`). |
