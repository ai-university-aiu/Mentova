...
| `arc_agi_3_guided.pl` | The **ARC-AGI-3_Guided** sub-project — the human-in-the-loop face of the ARC-AGI-3 Chat page. A human types clues; each is grounded into a Causalontology assertion and injected into the live game, and everything learned lands in the shared substrate (data lattice, Causalontology, J-Space) that the Solo sub-project reads back. |
| `arc_agi_3_solo.pl` | The **ARC-AGI-3_Solo** sub-project — takes the learnings the guided sessions wrote (goal, priorities, hazards, labels, causal relations, J-Space) and runs the ARC-AGI-3 game environments with no human direction, moment to moment, writing a date-and-time-stamped report of each attempt to `ARC-AGI-3_Solo_Attempts/`. On the ARC page: **[Guided Mode]**, **[Solo Mode]** (chat greyed out, view-only game with light-up action presses), **[Restart Game Environment]**, a game-environment dropdown, and an "ARC-AGI-3 Solo Attempts Record" page. Demo: 6/6 criteria PASS, including a human teaching the locksmith then Solo winning it unaided. See `docs/ARC-AGI-3_Guided_and_Solo_Modules_v1.txt`. |
