...
| `arc_agi_3_live.pl` | The **live ARC-AGI-3 client** — connects to the real environments at `https://three.arcprize.org` speaking the exact toolkit-confirmed protocol (`X-API-Key`; `GET /api/games`; `POST /api/scorecard/open`; `POST /api/cmd/RESET` and `/api/cmd/ACTION1..6`; states `NOT_PLAYED/NOT_FINISHED/WIN/GAME_OVER`). The **[Connect Live Environments]** button fills the dropdown with the real games; Guided and Solo then play them. Fully guarded: with no `ARC_API_KEY` it falls back to the three local stand-ins and says why. Tested end-to-end (6/6) against a mock server implementing the exact endpoints. To go live, set `ARC_API_KEY` (or `data/arc_api_key.txt`) — see `docs/ARC-AGI-3_Live_Connection_v1.txt`. |
