/*  Mentova — ARC-AGI-3 Game Knowledge Transfer Demonstration
    ...
    Acceptance criteria (each prints PASS or FAIL):
      ...
      AC-KT-006: the relation reached the causal predictor (co_predict).
...
run_arc3_knowledge_demo :-
    ...
    report('AC-KT-005',
        ( co_core:co_cro(_, [g(ls20, step_on(ring))], [refill(timer)],
    ...
    report('AC-KT-006',
        ( co_core:co_predict(g(ls20, step_on(ring)), refill(timer)) )),
    ...
    report('AC-KT-009',
        ( catch(jspace:js_reading(arc3_mind, R), _, fail), R \== [] )),
