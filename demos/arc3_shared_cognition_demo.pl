/*  Mentova — Shared Cognition Across Solo, Human-Guided, and AI-Guided Play
    ...
    (co_wm), the hypothesis-commitment pack (co_hypo), goal inference (co_goalinfer)
...
run :-
    ...
    catch(mentova_arc_chat:wm_reset, _, true),
    ...
    report('AC-SC-001',
        ( mentova_arc_chat:hy_committed(G, productive(ADown)),
          mentova_arc_chat:wm_stats(G, stats(_, T0)), T0 > 0,
    ...
    report('AC-SC-002',
        ( catch(mentova_arc_chat:wm_reset, _, true),
          ...
          mentova_arc_chat:wm_stats(G, stats(_, 0)) )),
    ...
    report('AC-SC-003',
        ( mentova_arc_chat:ma_load_learnings,
          mentova_arc_chat:hy_committed(G, productive(ADown)),
          mentova_arc_chat:wm_stats(G, stats(_, T1)), T1 > 0 )),
